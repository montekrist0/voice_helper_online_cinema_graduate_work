        ...
        for batch in self.extractor.extract_batch_from_database(
            ...
            if batch:
                ...
                try:
                    ...
                    logging.info('\tExtracted %s rows for %s', len(batch), self.config.elastic_index_name)
                    logging.info(
...
def get_etl_handlers(conn, configs: List[ETLConfig]) -> List[ETLHandler]:
    ...
    """
    return [ETLHandler(PostgresExtractor(conn.cursor()), ElasticsearchLoader(), config) for config in configs]
